...
#define LOOPR for(int k=0;k<N3+1;k++)for(int j=0;j<N2;j++)for(int i=0;i<N1;i++)
...
/* here we interpolate fluid variables to geodesic X just like in ipole */
	...
	for (i = 0; i < N1; i++) {
	  for (j = 0; j < N2; j++) {
		...
		bcon[i][j][k][3] * bcov[i][j][k][3] ;
	      ...
	      b2=beta*beta;
	      //trat_d=Rhigh, trat_j=Rlow
	      trat = trat_d * b2/(1. + b2) + trat_j /(1. + b2);
	      ////old// Thetae_unit=2.*MP/15./ME;
	      ...
	      //ceiling and floor on temperature
...
void init_harm3d_data(char *fname)
{
	...
	if(NPRIM>8){                                                                                                                                                                                              
          fstart[3] = 8;                                                                                                                                                                                          
          hdf5_read_array(p[KEL][0][0], "prims", 4, fdims, fstart, fcount, mdims, mstart, H5T_IEEE_F64LE,fname);                                                                                                  
          fstart[3] = 9;                                                                                                                                                                                          
          hdf5_read_array(p[KTOT][0][0], "prims", 4, fdims, fstart, fcount, mdims, mstart, H5T_IEEE_F64LE,fname);                                                                                                 
        }                                                                                                                                                                                                         
	...
	for(i = 0; i < N1; i++){
	  for(j = 0; j < N2; j++){
		...
		//fprintf(stderr,"%d %d %d %g %g %g \n",g,i,j,k,p[KRHO][i][j][k], ucon[i][j][k][1]);
		Ladv += g * p[UU][i][j][k] * ucon[i][j][k][1] * ucov[i][j][k][0] ;
