...
void gcov_func(double *X, double gcov[][NDIM])
{
  ...
  /* gcov_ks[0][0] = -(1. - 2. / (r * mu)); */
  /* gcov_ks[0][3] = -2. * a * s2 / (r * mu); */
  /* gcov_ks[3][0] = gcov_ks[0][3]; */
  /* gcov_ks[1][1] = mu / DD; */
  /* gcov_ks[2][2] = r2 * mu; */
  /* gcov_ks[3][3] = r2 * sth * sth * (1. + a2 / r2 + 2. * a2 * s2 / (r2 * r * mu)); */
...
void init_model(char *args[])
{
	...
	dx[1]=(stopx[1]-startx[1])/((double) N1);
	dx[2]=(stopx[2]-startx[2])/((double) N2);
	dx[3]=(stopx[3]-startx[3])/((double) N3);
...
void get_fluid_params(double X[NDIM], double gcov[NDIM][NDIM],
...
void get_model_ucon(double X[NDIM], double Ucon[NDIM])
{
  ...
  if(r>Risco){
    ...
    //here change for spherical polar
...
void get_model_bcon(double X[NDIM], double Bcon[NDIM])
{
  ...
  //TOROIDAL FIELD with sign flip in the center
  if(TOROIDAL){
    B_1=0.0;
    B_2=0.0;
    B_3=1.0;    
...
double make_spot(double X[NDIM],double spot_orbit, double spot_size, double variable_max)
{

    double sigma_spot,P,omega,r32;
    double xspot[NDIM];//curent position of a spot center in KS'                                                                                                           

    double xx;
    double xc,yc,zc,xs,ys,zs;
    int n,nmax;
    double spot_n_var;

    sigma_spot=spot_size/2.;

    r32=pow(spot_orbit,1.5);
    omega=1./(r32+a);
    P=2.*M_PI/omega;

    xspot[1]=log(spot_orbit);
    xspot[2]=0.5; //equator
    xspot[3]=fmod(X[0]/P,1.)*2.*M_PI; //fraction of a full period *2*pi, spot current phi at time t=X[0]
    nmax=1; //nmax spots del phi=2*pi/nmax, parameter
    ...
    for(n=0;n<nmax;n++){
      ...
      xspot[3] += 2*M_PI/nmax*n;
        ...
        //this is valid only for a small spot, locally spacetime flat

        xx=fabs(pow(xc-xs,2)+pow(yc-ys,2)+pow(zc-zs,2));

        spot_n_var=variable_max*exp(-(xx)/2./sigma_spot/sigma_spot);
        if(spot_n_var < 0.01*variable_max) spot_n_var=0.0; // Jesse's cut
...
double get_model_ne(double X[NDIM])
{
  ...
  bl_coord(X, &r, &th);
  double mu2=cos(th)*cos(th);
  double sigma2=0.3*0.3;

  if(r>Risco && r<Rout ){
    return 1e6*pow(r,-1.5)*exp(-mu2/2/sigma2) + make_spot(X,11.,3.,1e8);
  }else{
    return 0.0;
  }
