...
int main(int argc, char *argv[])
{
    ...
    rcam = 1000;
    phicam = 0.0; //deg
    Xcam[0] = 0.0; //time at the camera, for slow light number for fast light 0
    Xcam[1] = log(rcam);
    double x[NDIM] = {Xcam[0], rcam, thetacam/180.*M_PI, phicam/180.*M_PI};
    Xcam[2] = root_find2(x); //find X2 = (0,1)
    Xcam[3] = phicam/180.*M_PI; //radians
    fprintf(stderr,"camera coordinates: %g %g %g %g \n",Xcam[0],Xcam[1],Xcam[2],Xcam[3]);
    double r,th;
    bl_coord(Xcam,&r,&th);
    fprintf(stderr,"camera theta=: %g \n",th/M_PI*180.);
...
#pragma omp parallel for schedule(dynamic,1) collapse(2) shared(image,imageS,Xcam,fovx,fovy)
	...
	col_var[i][j][2]=X[0]; //final time recorded
...
void make_txt(double image[NX][NY],double imageS[NX][NY][NDIM], double freqcgs, char *fname, double FOVX, double FOVY, double scale)
{
  ...
  for (j = NY; j > 0; j--)
    for (i = 0; i < NX; i++)
      {
	//here QU are dumpted to follow ehtim convention
